...
private const val PAYMENT_ID = "660e8400-e29b-41d4-a716-446655440111"
...
class PaymentControllerTest {
    ...
    @MockkBean
    private lateinit var syncBookingPaymentUseCase: SyncBookingPaymentUseCase
    ...
    private fun performSync() =
        mockMvc.perform(
            post("/api/v1/bookings/$BOOKING_ID/payments/$PAYMENT_ID/sync")
                .contentType(MediaType.APPLICATION_JSON),
        )
    ...
    @Test
    fun `should return 401 when syncing a payment without authentication`() {
        performSync().andExpect(status().isUnauthorized)

        verify(exactly = 0) { syncBookingPaymentUseCase.sync(any()) }
    }

    @Test
    @WithMockUser(username = USER_ID)
    fun `should return 200 with the refreshed booking and payment statuses`() {
        every { syncBookingPaymentUseCase.sync(any()) } returns
            SyncBookingPaymentResult.Synced(
    ...
    @Test
    @WithMockUser(username = USER_ID)
    fun `should return 403 when the caller does not own the synced payment`() {
        every { syncBookingPaymentUseCase.sync(any()) } returns SyncBookingPaymentResult.NotOwner
    ...
    @Test
    @WithMockUser(username = USER_ID)
    fun `should return 404 when the synced payment is not found`() {
        every { syncBookingPaymentUseCase.sync(any()) } returns SyncBookingPaymentResult.NotFound
