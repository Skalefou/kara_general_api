...
class RoomControllerTest {
    ...
    fun `should return 200 with room without authentication`() {
        ...
        mockMvc
            .perform(get("/api/v1/rooms/$ROOM_ID"))
            .andExpect(status().isOk)
    ...
    fun `should return 400 when creating a room with an unknown service`() {
        ...
        every { createRoomUseCase.createRoom(any()) } returns
            CreateRoomResult.UnknownService(
                com.kara.kara_general_api.domain.model.service
                    .ServiceId(serviceId),
            )
        ...
        mockMvc
            .perform(
                post("/api/v1/rooms")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        """{"name":"S","description":"d","street":"r","city":"c","postalCode":"p","country":"F",""" +
    ...
    fun `should return 204 when admin deletes a room`() {
        ...
        mockMvc
            .perform(delete("/api/v1/rooms/$ROOM_ID"))
    ...
    fun `should return 202 with the image id when admin adds a room image`() {
        ...
        mockMvc
            .perform(multipart("/api/v1/rooms/$ROOM_ID/images").file(imageFile()))
    ...
    fun `should return 415 when the room image type is not supported`() {
        ...
        mockMvc
            .perform(multipart("/api/v1/rooms/$ROOM_ID/images").file(imageFile("text/plain")))
    ...
    fun `should return 204 when admin removes a room image`() {
        ...
        mockMvc
            .perform(delete("/api/v1/rooms/$ROOM_ID/images/${UUID.randomUUID()}"))
            .andExpect(status().isNoContent)
    }
